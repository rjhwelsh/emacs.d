...
**** Add a view for remaining todo items without persistent tags
    ...
    #+end_src
*** Tag-Tree (sparse) search buffers
**** Tag-tree prefix
     #+begin_src emacs-lisp
       (setq org-agenda-tag-tree-prefix "b") ;; b for buffer search
       (add-to-list 'org-agenda-custom-commands
		    `(,org-agenda-tag-tree-prefix . "Tag-tree search of current buffer"))
     #+end_src
**** Sparse tree of *any persistent tags* in current buffer 
    #+begin_src emacs-lisp
      ;; Tags tree
      (add-to-list 'org-agenda-custom-commands
		   `(,(concat org-agenda-tag-tree-prefix "p") 
...
**** Sparse tree of anything *without* a persistent tag
    ...
    #+begin_src emacs-lisp
      ...
      (add-to-list 'org-agenda-custom-commands 
		   `(
		     ,(concat org-agenda-tag-tree-prefix "u") 
