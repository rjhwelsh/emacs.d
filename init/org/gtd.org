...
#+begin_src emacs-lisp
  (defun org-agenda-skip-subtree-if-regexp (skip-re &optional invert)
...
#+begin_src emacs-lisp
  (defun org-agenda-skip-subtree-if-tags (tags &optional invert)
  ...
  Tags is a list of tags"
    (let* ((tags-re (cond ((null tags) nil)
      ...
      (org-agenda-skip-subtree-if-regexp skip-re invert)
      ))
  #+end_src
***** Skip sub-tree based on todo keywords present
#+begin_src emacs-lisp
  (defun org-agenda-skip-subtree-if-todo (todo &optional invert)
...
**** Make a list of all projects
     #+begin_src emacs-lisp
       (add-to-list 'org-agenda-custom-commands
		    '("l" "List of projects" tags (nth 0 org-stuck-projects)
		      ((org-agenda-overriding-header "List of projects")
		       (org-agenda-skip-function 
			;; Skip entry if it does not contain todo keywords that define a project
			'(org-agenda-skip-subtree-if-todo 		       
			 (nth 1 org-stuck-projects) 
			 t))
