...
*** Hooks
Refile hooks to operate on entries that have just been refiled.
(And *before* stuff has been deleted at the old location)
#+begin_example emacs-lisp
(add-hook org-after-refile-insert-hook (lambda (x) (message "helloworld")))
#+end_example

Display current outline-path.
#+begin_example emacs-lisp
...
Define function to set property value.
#+begin_src emacs-lisp
  (defun post-refile-function ()
    "Sets last-refile property to X"
    (org-set-property
     "last-refile"
     (org-display-outline-path t t nil t))
    )
(setq org-after-refile-insert-hook nil)
#+end_src
...
#+begin_src emacs-lisp
(setq org-before-refile-insert-hook 'post-refile-function)
#+end_src
