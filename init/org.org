...
#+BEGIN_SRC emacs-lisp
	(setq org-agenda-sorting-strategy
				'((agenda time-up deadline-up scheduled-up todo-state-up category-keep priority-down effort-up tag-up)
					(todo           deadline-up              todo-state-up category-keep priority-down effort-up tag-up)
					(tags           deadline-up              todo-state-up category-keep priority-down effort-up tag-up)
					(search         deadline-up              todo-state-up category-keep priority-down effort-up tag-up)))
#+END_SRC
...
*** Faces
		These define faces for org-mode.
		Multiple groups are specified using [[info:elisp#Common%20Keywords][:group]] more than twice.
